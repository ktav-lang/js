...
`stringifyForceStrings` 的输出与 `dumps` 相同，但会把每个叶子标量 ——
integer、float、boolean、`null` —— 用原始标记(`::`)压平为文本形式。
复合值保持其结构，结果经由 `loads` 解析回来仍是同一组 String 标量。
...
自 ktav 0.7.2 起还有 `message`：信封自身的第十个字段，逐字取用，绝不由
